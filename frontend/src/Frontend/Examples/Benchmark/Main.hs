...
module Frontend.Examples.Benchmark.Main
  (app)
  where
...
import           Data.FastMutableIntMap hiding (new)
...
app :: (DomBuilder t m, MonadFix m, MonadHold t m) => m ()
...
-- app = el "asdf" $ headW >> bodyW 54321234
app = headW >> bodyW 543
-}
...
bodyW :: (DomBuilder t m, MonadHold t m, MonadFix m) => Entropy -> m ()
...
bodyW seed = divClass "main" $ divClass "container" $ mdo
  ...
  -- let initial = Model { rng = StdGen 123 123, nextNum = 1, selection = Nothing }
...
tableW :: (DomBuilder t m, MonadHold t m) => Event t TableDiff -> m (Event t (IntMap Step))
tableW diff = do
